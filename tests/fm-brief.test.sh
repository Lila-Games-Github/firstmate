...
test_scout_and_secondmate_scaffold() {
  ...
  # Which review surface a scout is told to use now depends on the lavish-axi
  # build bootstrap finds at scaffold time, so this scaffold check asserts the
  # invariant that holds either way: the brief always tells the scout how to put
  # its findings in front of the captain. The branch itself is pinned, with
  # faked versions, by test_scout_lavish_line_follows_presentation_floor.
  grep -q 'you may host the Lavish review loop yourself' "$brief" \
    || grep -q 'deliver your findings as a text report without Lavish' "$brief" \
    || fail "scout brief named neither review surface: no Lavish loop offer and no text-report instruction"
  assert_grep "learning-candidate-lifecycle/SKILL.md" "$brief" \
...
# Lane mode must not leak into a non-lane brief. This used to compare each mode's
# whole generated brief against a committed fixture, but those fixtures are a
# fork-only path and this fork-sync merge may not rewrite one, so they cannot be
# regenerated against upstream's reshaped brief from inside the merge. Assert the
# boundary directly instead: a non-lane brief keeps the crewmate branch convention
# and carries no lane vocabulary at all, in every delivery mode.
# Follow-up: regenerate tests/fixtures/fm-brief/*.md with FM_BRIEF_FIXTURE_UPDATE=1
# in an ordinary PR and restore the whole-brief comparison on top of this.
test_non_lane_ship_briefs_carry_no_lane_vocabulary() {
  local home mode brief lane_marker
  home="$TMP_ROOT/non-lane-boundary"
  mkdir -p "$home/data"
  for mode in no-mistakes direct-PR local-only; do
    FM_HOME="$home" FM_ROOT_OVERRIDE=/FM_ROOT FM_CLASSIFY_PAUSED_VERB=paused \
      "$ROOT/bin/fm-brief.sh" "boundary-$mode" fixture-project --mode "$mode" >/dev/null 2>&1 \
      || fail "$mode: non-lane brief should scaffold"
    brief="$home/data/boundary-$mode/brief.md"
    assert_grep "Delivery contract: mode=$mode" "$brief" \
      "$mode: non-lane brief lost its delivery contract line"
    assert_grep "git checkout -b fm/boundary-$mode" "$brief" \
      "$mode: non-lane brief lost the crewmate branch convention"
    for lane_marker in "## Lane overrides" "Playbot lane workspace" \
      "fm-lane-base-check.sh" "never create or switch branches" \
      "Delivery contract - READ THIS BEFORE YOU SHIP ANYTHING"; do
      if grep -qF -- "$lane_marker" "$brief"; then
        fail "$mode: non-lane brief leaked lane text: $lane_marker"
      fi
    done
  done
  pass "fm-brief.sh: non-lane ship briefs keep the crewmate convention and no lane vocabulary"
}
