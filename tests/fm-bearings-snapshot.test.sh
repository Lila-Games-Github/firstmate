...
test_oversized_parsed_backlog_survives_all_public_snapshot_routes() {
  ...
  record_claude_state "$home/state" active-large idle
  ...
  summary=$(PATH="$fakebin:$PATH" FM_HOME="$home" "$ROOT/bin/fm-fleet-snapshot.sh" --secondmate-home-summary)
  # One conjunction cannot say which half of "content or classification" broke,
  # and this case is the one that fails on a platform the author is not on, so
  # each clause reports itself with the shape it actually saw.
  printf '%s' "$summary" | jq -e '.schema == "fm-secondmate-home-summary.v1"' >/dev/null \
    || fail "secondmate summary wrong schema: $(printf '%s' "$summary" | jq -c '{schema,valid}' 2>&1 | head -c 400)"
  printf '%s' "$summary" | jq -e '.valid == true' >/dev/null \
    || fail "secondmate summary not valid: $(printf '%s' "$summary" | jq -c '{valid,reason,invalidity,state,counts}' 2>&1 | head -c 600)"
  printf '%s' "$summary" \
    | jq -e '.active_children | any(.id == "active-large" and (.doing | startswith("status-prefix-retain-large-body-")))' >/dev/null \
    || fail "secondmate summary lost the oversized active child's doing: $(printf '%s' "$summary" | jq -c '[.active_children[]? | {id,doing:((.doing // null) | if . == null then null else .[:80] end)}]' 2>&1 | head -c 600)"
  printf '%s' "$summary" | jq -e --arg marker "$marker" \
    '.queued | any(.id == "large-marker" and (.title | startswith($marker[:120])))' >/dev/null \
    || fail "secondmate summary lost the oversized queued title: $(printf '%s' "$summary" | jq -c '[.queued[]? | {id,title:((.title // null) | if . == null then null else .[:80] end)}]' 2>&1 | head -c 600)"
  printf '%s' "$summary" | jq -e '.counts.queued == 1' >/dev/null \
    || fail "secondmate summary miscounted queued: $(printf '%s' "$summary" | jq -c '.counts' 2>&1 | head -c 400)"

  valid_mate="$TMP_ROOT/oversized-status-valid-home"
  make_valid_secondmate_home oversized-status-valid "$valid_mate"
  # A child is only "structured-home" when it has published a home-summary
  # ledger; the snapshot reads that file rather than sampling the child live.
  # The oversized content this case exercises still arrives through the parent
  # event copied below, which is what the assertions read.
  write_remote_home_summary "$valid_mate" 1000
  append_secondmate_registry "$home" oversized-status-valid "$valid_mate"
  fm_write_secondmate_meta "$home/state/oversized-status-valid.meta" "$valid_mate" \
    "firstmate:fm-oversized-status-valid" sample
  cp "$home/state/active-large.status" "$home/state/oversized-status-valid.status"
  invalid_mate="$TMP_ROOT/oversized-status-missing-home"
  fm_write_secondmate_meta "$home/state/oversized-status-fallback.meta" "$invalid_mate" \
    "firstmate:fm-oversized-status-fallback" sample
  cp "$home/state/active-large.status" "$home/state/oversized-status-fallback.status"
  ...
  printf '%s' "$canonical" | jq -e --arg marker "$marker" --arg status_tail "$status_tail" '
    .schema == "fm-fleet-snapshot.v1"
      and .main_inventory.valid == true
      and (.tasks | any(.id == "active-large" and .backlog.id == "active-large"
        and (.paths.status_log.last_event.raw | length) > 131072
      ...
      and (.secondmate_current.records | any(.id == "oversized-status-fallback"
  ...
  printf '%s' "$bearings" | jq -e '
    .schema == "fm-bearings.v1"
      and (.in_flight | any(.id == "active-large" and .kind == "ship"
        and (.doing | startswith("status-prefix-retain-large-body-"))))
...
# This route must stay ahead of every invocation below: its contract is to run
# the oversized case alone, under its own timeout, and nothing else. Upstream's
# cases arriving above it would silently turn one isolated test into seven.
