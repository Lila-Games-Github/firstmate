...
SH
  ...
  (
    ...
    # Loaded inside this background subshell only. Following it statically makes
    # ShellCheck treat `!` as subshell-modified for the whole file, which then
    # misreports every later `pid=$!` capture (SC2031); the library is a runtime
    # dependency of this fixture, not a static one.
    # shellcheck source=/dev/null
    . "$ROOT/bin/fm-wake-lib.sh"
    # Read by fm_lock_acquire_wait in the library loaded just above, which is
    # not followed statically, so ShellCheck cannot see the use.
