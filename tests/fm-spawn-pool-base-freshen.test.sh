...
add_task_record() {  # <id>
  ...
  # bin/fm-spawn.sh refuses a brief whose task subsections are missing or empty,
  # so the fixture carries the shape a real scaffold produces.
  cat > "$HOME_DIR/data/$1/brief.md" <<EOF
# Task
## Captain's intent
Exercise pooled base freshening for $1.

## Firstmate spec
Launch the task from the expected base and record the result.
EOF
}
