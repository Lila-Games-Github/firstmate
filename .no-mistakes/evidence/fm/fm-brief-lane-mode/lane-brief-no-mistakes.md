...
Implement and commit, then report done and WAIT: firstmate tells you when to run /no-mistakes. Do not start the pipeline before it does.
...
# Rules
1. Never push to the default branch. Never merge a PR. Work only on the branch your workspace was created on; never create or switch branches.
...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/lane-evidence.6nGFUU/home/state/frog-pile-lane-43.status'`
