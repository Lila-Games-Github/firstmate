...
# Setup
You are in a disposable git worktree of some-proj, at a detached HEAD on a clean copy of your task's base branch (its recorded landing branch, else the default branch).
...
1. First action: create your branch: `git checkout -b fm/lane-task-a1`

# Rules
1. Never push to any remote and never open a PR. Work only on your `fm/lane-task-a1` branch; firstmate handles the merge into your recorded landing branch (the default branch when none is recorded).
...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-brief-evidence/state/lane-task-a1.status'`
...
# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/home/sanchith/.no-mistakes/worktrees/f62841b885f5/01M1M9CN03TT6BR120EP32YVK4/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md` that lacks `## Maintaining this file`, add that short self-governance section from `/home/sanchith/.no-mistakes/worktrees/f62841b885f5/01M1M9CN03TT6BR120EP32YVK4/bin/fm-ensure-agents-md.sh` in the same pass.
Keep it proportionate: skip `AGENTS.md` edits for trivial tasks that produced no durable project knowledge.

# Definition of done
Delivery contract: mode=local-only
This task ships **local-only**: no remote, no PR, no pipeline.
The task is complete only when committed on your branch `fm/lane-task-a1`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto your recorded landing branch - the `landing_branch=` firstmate recorded for this task in `'/tmp/fm-brief-evidence/state/lane-task-a1.meta'` (contract: bin/fm-spawn.sh's header), falling back to the default branch only when none is recorded.
If that landing branch has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append `done: ready in branch fm/lane-task-a1` to the status file and stop.
