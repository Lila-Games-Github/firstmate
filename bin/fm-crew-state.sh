...
#      through to the routed status log, but a checks-green event has no local
#      run to attribute it to and reports unknown instead; dead/missing report
#      the remote verdict; an unreachable or unreadable remote reports
...
# A checks-green status event is only as good as the CI verdict currently
# attributable to this crew's own branch and code. Full attribution, a running CI
# step and an explicit green marker are all required; a coarse attribution, a
# fixing run, or an unknown, empty or unreadable verdict leaves the event
# unverified, so the crew keeps reporting the state its run actually supports.
...
if [ -n "$REMOTE_HOST" ]; then
  ...
  case "$REMOTE_STATE" in
    alive)
      # A remote mate's checks-green event is the one status line this host
      # cannot attribute to a current run: the no-mistakes run that produced it
      # lives on the remote host, so there is no local run record to verify it
      # against. Report it as unknown rather than rendering an unverifiable
      # event as a finished, review-ready PR. Every other verb is ordinary
      # activity and still reads straight from the routed status log.
      if [ -n "$LOG_VERB" ] && ! log_reports_ci_ready; then
...
# A "no CI checks reported" marker is an ABSENT verdict, never a passing one, so
# it reads not-ready in either its terminal or its pending wording.
nm_ci_checks_state() {
