...
family_for_basename() {
  case "$1" in
    ...
    fm-crew-state.test.sh|fm-captain-hold-lifecycle.test.sh|fm-learning-candidate.test.sh|\
    ...
    fm-lint-workflows.test.sh|fm-stock-bash-compat.test.sh|\
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-check-unregister.test.sh|fm-merge-local.test.sh|fm-pr-check-security.test.sh|\
    fm-pr-fork-sync-proof.test.sh|fm-pr-merge.test.sh|\
    ...
    fm-dispatch-resolve.test.sh|fm-jev.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-gate-refuse*|bin/fm-lock*|bin/fm-session-lock-lib.sh|\
    ...
    bin/fm-dispatch-resolve.sh)
      printf '%s\n' "__script__:fm-dispatch-resolve.test.sh"
      ;;
    bin/fm-jev*.sh|bin/jev-questions/*|tests/jev-http-server.py|tests/fixtures/jev-ledger.jsonl)
      printf '%s\n' "__script__:fm-jev.test.sh"
      ;;
    bin/fm-env-lib.sh)
      # The one .env accessor, sourced by bin/fm-x-lib.sh (Relay token) and
      # bin/fm-dispatch-resolve.sh and bin/fm-jev.sh (TYPESAFE_API_KEY).
      printf '%s\n' pr-forge
      printf '%s\n' "__script__:fm-dispatch-resolve.test.sh"
      printf '%s\n' "__script__:fm-jev.test.sh"
      ;;
    ...
    bin/fm-install-herdr.sh|bin/fm-install-treehouse.sh|bin/fm-herdr-ci-cleanup.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-lint.sh|bin/fm-lint-workflows.sh|bin/fm-stock-bash-compat.sh|bin/fm-install-shellcheck.sh|\
