...
last_status_line() {  # <status-file> [<previous-event-var>]
  ...
  # The scan is exactly two lines, so the latest event is everything after the
  # first newline. Take it with the shortest-match "#": bash's longest-match
  # "##" is quadratic in the subject, and one 132 KB status line cost 6.0s here
  # against fm-fleet-snapshot.sh's 10s per-task budget - enough to report a live
  # crew's current state as unknown on a loaded host.
  printf '%s\n' "${scan#*$'\n'}"
}
