...
# trust record, PR or lane sidecar, and publication record with the rest of
# the volatile state, while holding the shared per-task check-publication lock.
...
# Not followed, for the reason bin/fm-watch.sh states at its own PR-library
# source: ShellCheck's extended analysis blows up combinatorially on this pair
# and is OOM-killed above 8 GB, while suppressing just this one edge takes the
# file from killed-at-47s to clean-at-25s. Measured against the other 14 source
# edges here, which are all innocent. The library is a lint root itself, so this
# costs no coverage.
# shellcheck source=/dev/null
...
# Defined ahead of remote_secondmate_teardown below, which calls them: this
# fork retires a remote secondmate's poll artifacts as part of that teardown,
# so the helpers must already exist when that call runs.
...
# Holds the shared per-task check-publication lock across the whole removal, so
# a Playbot lane poll cannot arm state/<id>.check.sh between the validation and
...
remove_pr_poll_artifacts() {
  ...
  if [ "$status" -eq 0 ]; then
    fm_pr_poll_merge_notified_remove "$state_dir" "$id" || status=1
  fi
  if [ "$status" -eq 0 ]; then
    rm -f "$state_dir/$id.check.sh" "$state_dir/$id.pr-poll" \
      "$state_dir/$id.pr-poll-registration" "$state_dir/$id.pr-poll-retirement" \
      "$state_dir/$id.merge-authority" "$state_dir/$id.check-trust" \
      "$state_dir/$id.lane-poll" || status=1
  fi
...
remote_secondmate_teardown() {
  local remote_host remote_root remote_home kind route_host route_root route_home out rc tmp status=0
  ...
  handoff_wake_retire \
    || { echo "error: remote receiver wake cleanup failed; preserving the local route for retry" >&2; status=1; }
  if [ "$status" -eq 0 ]; then
  ...
  if [ "$status" -eq 0 ]; then
    fm_backlog_atomic_transition remove "$STATE/$ID.meta" "task record" "$STATE" || status=1
  fi
  if [ "$status" -eq 0 ]; then
    rm -f -- "$STATE/$ID.turn-ended" "$STATE/$ID.progress" || status=1
  fi
  fm_pr_poll_lock_release || status=1
...
# Released only now that the record itself is gone. A concurrent
# bin/fm-pr-check.sh has already rewritten and released this task's metadata by
# the time it publishes, so the publication lock is the only thing left holding
# it off; releasing before the retirement lets it arm a poll for a task whose
# record is being removed, and the check outlives the task it names.
