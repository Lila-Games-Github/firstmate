...
detect_own() {
  local marker ancestry strength harness windows_harness
  marker=$(harness_marker)
  ancestry=$(harness_ancestry)
  if [ -z "$ancestry" ]; then
    if [ -n "$marker" ]; then echo "$marker"; return; fi
    # A Windows harness publishes no POSIX-visible ancestry, so ask the Windows
    # process tree before reporting unknown.
    windows_harness=$(fm_windows_harness_ancestry_name 2>/dev/null || true)
    if [ -n "$windows_harness" ]; then echo "$windows_harness"; return; fi
    echo unknown
    return
  fi
  strength=${ancestry%% *}
