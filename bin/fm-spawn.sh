...
#   the current remote default branch, and resets to that tip (the
#   --landing-branch paragraph above owns the base-selection mechanics). When no
#   origin is detected, spawn skips that remote freshness check and launches from
#   the clean worktree's current HEAD, but a task that records a landing branch
#   refuses instead of launching from an unverified base. Relaunch reuses the recorded worktree without
...
for a in "$@"; do
  ...
  --landing-branch=*)
    LANDING_BRANCH=${a#--landing-branch=}
    LANDING_SET=1
    ;;
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  [ "$LANDING_SET" -eq 0 ] || {
    echo "error: --relaunch preserves the task's recorded landing branch; correct it with bin/fm-landing-branch.sh instead" >&2
    exit 1
  }
else
...
freshen_spawn_worktree_base() { # <worktree>
  ...
  if ! spawn_worktree_has_origin_config "$worktree"; then
    # No origin means no remote base to refresh against, which is a launch from
    # the clean worktree's HEAD. A recorded landing branch cannot be resolved or
    # verified that way, and it must never fall back, so it refuses instead.
    [ -z "$LANDING_BRANCH" ] || {
      echo "error: task records landing branch '$LANDING_BRANCH' but pooled worktree '$worktree' has no origin configuration to resolve it against; refusing to launch from an unverified base" >&2
      return 1
    }
    return 0
  fi
