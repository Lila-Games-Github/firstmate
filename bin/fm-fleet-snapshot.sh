...
task_json_lines() {
  ...
  while [ "$index" -lt "$SNAPSHOT_TASK_META_COUNT" ]; do
    ...
    # The status log's own text is unbounded, and Linux caps each argv element at
    # 128 KiB, so every value that can carry it travels on stdin. Bounded scalars
    # and paths stay as arguments.
    printf '%s\n%s\n%s\n%s\n' \
      "$current_json" "$status_json" "$open_decisions_json" \
      "$(printf '%s' "$last_event_raw" | jq -Rs .)" | jq -s \
      --arg id "$id" \
      --arg kind "$kind" \
      --arg harness "$harness" \
      --arg mode "$mode" \
      ...
      '.[0] as $current_state | .[1] as $status_log | .[2] as $open_decisions
      | .[3] as $last_event_raw
      | {
        id:$id,
        kind:$kind,
...
parent_evidence_reconciliation_json() {  # <summary-json-file> <activities-json> <decisions-json>
  printf '%s\n%s\n' "$2" "$3" | jq -s --slurpfile summary "$1" '
    .[0] as $activities | .[1] as $decisions
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      ...
      terminal_contradiction=$(printf '%s\n%s\n' "$reconciliation" \
        "$(printf '%s' "$event_note" | jq -Rs .)" | jq -s -r '
        .[1] as $note
      ...
      # Scans, reconciliation and the child's own event text are unbounded, so
      # they travel on stdin; bounded scalars stay as arguments.
      printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
        "$decisions" "$activities" "$activity_scan" "$reconciliation" "$terminal" \
        "$(printf '%s' "$event_raw" | jq -Rs .)" \
        "$(printf '%s' "$event_note" | jq -Rs .)" | jq -s \
        ...
        .[0] as $decisions | .[1] as $activities | .[2] as $activity_scan | .[3] as $reconciliation
        | .[4] as $terminal | .[5] as $event_raw | .[6] as $event_note
        | ($summary[0]) as $summary
        |
      ...
      # A child's own event text and scans are unbounded, so they travel on stdin
      # rather than argv; bounded scalars and the slurped summary stay as before.
      printf '%s\n%s\n%s\n%s\n%s\n%s\n' \
        "$(printf '%s' "$event_raw" | jq -Rs .)" "$(printf '%s' "$event_note" | jq -Rs .)" \
        ...
        .[0] as $event_raw | .[1] as $event_note | .[2] as $activities | .[3] as $activity_scan
        | .[4] as $decisions | .[5] as $terminal
        | ($summary[0]) as $summary
        |
