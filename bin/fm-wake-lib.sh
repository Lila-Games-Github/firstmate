...
fm_lock_prepare_owner() {
  local ownerdir=$1 mypid back identity identity_back
  ...
  if [ "${FM_LOCK_REQUIRE_IDENTITY:-0}" = 1 ]; then
    ...
    [ "$identity_back" = "$identity" ] || return 1
  fi
}
...
fm_lock_points_to_owner() {
  ...
  if [ -d "$lockdir" ] && [ ! -L "$lockdir" ]; then
    actual=$(cat "$lockdir/owner" 2>/dev/null) || return 1
    [ "$actual" = "$ownerdir" ]
    return
  fi
...
fm_lock_owner_dir_for_path() {
  local lockdir=$1 ownerdir
  if [ -L "$lockdir" ]; then
    fm_lock_link_owner "$lockdir"
    return
  fi
  ownerdir=$(cat "$lockdir/owner" 2>/dev/null || true)
  [ -n "$ownerdir" ] || return 1
  printf '%s\n' "$ownerdir"
}

fm_lock_owner_alive() {
  local lockdir=$1 pid=$2 ownerdir expected actual
  fm_pid_alive "$pid" || return 1
  ownerdir=$(fm_lock_owner_dir_for_path "$lockdir" 2>/dev/null || true)
  [ -n "$ownerdir" ] || return 0
  [ -f "$ownerdir/pid-identity" ] && [ ! -L "$ownerdir/pid-identity" ] || return 0
  expected=$(cat "$ownerdir/pid-identity" 2>/dev/null || true)
  [ -n "$expected" ] || return 0
...
fm_lock_try_create() {
  ...
  if fm_lock_uses_directory_owner; then
    ...
    if printf '%s\n' "$mypid" > "$lockdir/pid" 2>/dev/null \
      && printf '%s\n' "$ownerdir" > "$lockdir/owner" 2>/dev/null; then
...
fm_lock_try_acquire() {
  local lockdir=$1 recovery_depth=${2:-0} pid steal cur rc steal_owner primary_owner current
  FM_LOCK_HELD_PID=
  FM_LOCK_OWNER_DIR=
  ...
  #
  # Bound that recursion without suppressing a legitimate first recovery: only a
  # nested attempt short-circuits. The outer attempt must still run recovery,
  # because a create can also fail when the lock's own directory moved under it,
  # and a caller that skips the task on failure would then never reach its
  # post-lock record check (tests/fm-backlog-atomicity.test.sh pins that refusal).
  if [ "$recovery_depth" -gt 0 ] && [ ! -e "$lockdir" ] && [ ! -L "$lockdir" ]; then
    return 1
  fi
  ...
  if [ -L "$lockdir" ]; then
    ...
    primary_owner=$(cat "$lockdir/owner" 2>/dev/null || true)
  fi
  cur=$(cat "$lockdir/pid" 2>/dev/null || true)
