...
fm_task_identity_retired() {
  ...
  if ! declare -F fm_meta_get >/dev/null 2>&1; then
    # Both loads here are CONDITIONAL runtime fallbacks, taken only when the
    # caller has not already sourced the owner, so they are not static
    # dependencies of this library. Following them statically pulls their whole
    # graph into every consumer's analysis, which makes ShellCheck misread an
    # unrelated string assignment in bin/fm-tool-update-check.sh as arithmetic
    # (SC2100 on `CHECK_ID=tool-updates`). Pointing the directive at /dev/null
    # keeps the lazy load honest about being a runtime path.
  ...
  if ! declare -F secondmate_registry_line_for_id >/dev/null 2>&1; then
    # shellcheck source=/dev/null  # conditional runtime fallback, as above
...
# fm_pr_poll_publish_prepared below additionally holds the cross-owner check
# publication lock, which is what keeps a Playbot lane poll and this PR poll
# from both claiming state/<id>.check.sh.
