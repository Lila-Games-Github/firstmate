...
# Same treatment, same reason, measured: this runtime grew to 138 KB upstream
# while the fork's PR library grew independently, and ShellCheck's extended
# analysis blows up combinatorially on the pair - OOM-killed above 8 GB, where
# either side alone analyses in ~35s. It is a lint root itself, so this costs no
# coverage; only the cross-file inference on this one edge is skipped.
# shellcheck source=/dev/null
