...
| Effort flag | `-c 'model_reasoning_effort="<low\|medium\|high\|xhigh\|max>"'`, verified on codex-cli 0.142.1 whose installed schema contains `model_reasoning_effort` and active config uses it. `low`/`medium`/`high`/`xhigh` are passed unconditionally. `max` is scoped to whatever the installed catalog at `${CODEX_HOME:-~/.codex}/models_cache.json` advertises for the REQUESTED model (`bin/fm-codex-catalog-lib.sh`, read fresh at spawn time, never a hard-coded model list): codex-cli 0.155.1's catalog advertises `max` for `gpt-6-luna`, `gpt-6-sol`, `gpt-6-astra`, `gpt-5.6-sol`, and `gpt-5.6-terra` in addition to `gpt-5.6-luna`. A model the catalog doesn't list `max` for, or an unreadable/missing catalog, REFUSES the spawn with an actionable error (`bin/fm-spawn.sh`) rather than silently launching at the catalog's default effort - a silent downgrade would leave a worker running at the wrong effort with no signal that it happened. |
