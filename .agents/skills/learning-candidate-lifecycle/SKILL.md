...
The lane where the signal occurred records one candidate before its terminal completion report when practical.
Workers use the absolute, home-bound capture invocation in their generated brief.
For a signal originating in a primary or secondmate supervisor lane, resolve the absolute active home from the session-start heading and the absolute tracked code root from this loaded skill's path, then run `FM_HOME=<absolute-active-home> <absolute-code-root>/bin/fm-learning-candidate.sh capture ...`; replace both placeholders with those absolute paths and use a privacy-safe supervisor-lane slug for `--task`.
