...
4. Call `retire_workspace` for that one id with the same explicit `landingBranch`, the same `registryProject`, and `confirm: true`.
   ...
   When the captain has explicitly authorized discarding that workspace's local changes, and its `discard.possible` evidence is true, pass `discardLocalChanges` with the captain's words verbatim as `authorization`, only the blocker codes the captain's words cover in `allow`, and, for `unlanded-commits` or `prune-would-drop-unlanded-head`, every exact commit id from the fresh evidence's `discard.unlandedCommits` in `commits`.
