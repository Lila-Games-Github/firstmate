...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and TypeSafe AI key TYPESAFE_API_KEY (presence-gates typed dispatch plus the active-by-default Jev observers; docs/configuration.md "Typed dispatch resolution" and "Jev decision observers"); LOCAL, gitignored
...
config/jev.json   optional Jev observer mode and budget overrides; absent uses active built-in defaults; LOCAL, gitignored, and not inherited; see docs/configuration.md "Jev decision observers"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  captain-questions.md  this home's captain-readable list of every open question waiting on the captain; LOCAL, gitignored, and never inherited
...
state/               runtime records and signals; gitignored
  ...
  learning-candidates/ private durable incident records that survive task cleanup; bin/fm-learning-candidate.sh owns their schema, lifecycle, and bounded summary, while learning-candidate-lifecycle owns semantic capture and curation
...
An `ABSENT` captain, shared-captain, secondmate, or learnings file means the firstmate repo's built-in defaults, no shared captain preferences, no registered secondmates, or no captured learnings; an `ABSENT` captain-questions file means this home has no open questions recorded; rebuild an absent or stale project registry from the clones before dispatch.
...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the away posture (`state/.afk-contract`, plus the `state/.afk` daemon flag where a daemon runs); a bounded unresolved learning-candidate summary when present; and one cheap alive/dead read of each task's recorded backend endpoint.
...
7. **Context digest and next step** - last of the bulk sections, the full contents of `data/projects.md`, `data/secondmates.md`, `data/captain.md`, `data/captain-shared.md`, `data/captain-questions.md`, and `data/learnings.md`, each clearly delimited, followed by the closing reminder.
...
After an autonomous merge, give the captain a one-line full-URL or local-branch outcome.
...
`data/captain-questions.md` is each home's readable face of everything firstmate is waiting on the captain for, while `data/backlog.md` remains the durable record with its hold reasons.
Raising a question with the captain adds its entry in the same turn, and the captain answering one deletes that entry in the same turn rather than marking it answered.
Apply the translation rules above to every entry: include no task ids, internal vocabulary, or raw paths the captain does not need.
...
If a task is dispatched into a Playbot lane, scaffold with `--lane` and its required `--landing-branch` rather than countermanding the generated brief on dispatch; a lane brief needs no dispatch-time override.
...
When the captain says the exact phrase `Ahoy Playbot!`, invokes `$ahoy-playbot`, or asks to initialize or repair Playbot lanes, load the `ahoy-playbot` skill.
