...
A part can still fail on its own - a timeout, an HTTP error, a malformed response - after the split has started. An aggregate verdict (`all_noul`, `any_noul`) is a claim about every named question, so it cannot be computed from the parts that succeeded and the whole consultation is then unavailable with that part's reason. A per-key verdict (`choices`) keeps the answers it has: `answers` holds exactly the questions Jev answered and `parts_unavailable` names why the rest are missing, and no caller may read an absent key as an answer.
