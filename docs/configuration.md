...
## Captain questions (data/captain-questions.md)

Each home's gitignored `data/captain-questions.md` is the captain-readable register of its open questions and is printed in full in the session-start context digest.
`AGENTS.md` section 9 owns its maintenance and captain-facing translation contract.
...
`consult` enforces that budget itself for every adapter, so no adapter can turn an oversized request into a silent no-op. An envelope larger than the budget is split into one request per question group - one verdict question plus every request question sharing its state key - each carrying only its own state, but only when its verdict is per key. An aggregate verdict such as the acceptance check's `all_noul` is one claim about every named question, so a part of it is not a consultation in its own right and would record a row asserting the whole subject's verdict over a subset; those envelopes are always one shortened request. Each part of a split sets its own `subject` to `<subject>#<group>`, so `finalize` matches one row and the report counts each subject exactly once. An envelope may declare `ledger.context`, an object mapping a shared state container to the field of each group's state that names the entry it needs; the open-question sweep uses `{"pages": "page"}` so each part carries only the pages its own questions cite rather than the whole container. A part still over budget has its longest state string shortened and records `truncated: true`. A request that cannot fit even then is refused with `per-call-token-cap`, or with `question-block-token-cap` when the pinned model and the question block already exceed the cap on their own and no shortening of the material could help, and the refusal is written to the ledger rather than dropped.
Everything a part carries that its own group does not own is shared: state copied verbatim plus the `ledger.context` entries that group cites. What the split duplicates is the sum of those per-part shared bytes less the distinct shared bytes the parts cover between them, and the split is taken only when the group-owned material is what makes the envelope oversized and that duplication stays inside one budget: the duplicated bytes must not exceed the group-owned bytes, and they must not exceed the per-call budget.
Ten questions citing ten different pages duplicate nothing and still split; ten questions citing one 20 KB page duplicate that page nine times over and are answered by a single shortened request instead.
The split is also refused when its parts would not fit the day's remaining calls and spend for that use, since a split that stops partway leaves its later questions unanswered.
A part can still fail on its own - a timeout, an HTTP error, a malformed response - after the split has started. Because only per-key verdicts split, the surviving parts keep the answers they have: `answers` holds exactly the questions Jev answered and `parts_unavailable` names why the rest are missing, and no caller may read an absent key as an answer.
Every one of those refusals sends one request with its longest state string shortened instead, so a verbose worker report shared by five acceptance criteria costs one shortened request rather than five near-identical ones.
...
Every budget refusal appends a ledger row with `network_attempted: false`, zero cost and zero tokens, so `bin/fm-jev-report.sh` can name why nothing ran instead of showing an empty report. Those rows consume no call or spend budget and are excluded from the metrics table, appearing under `unanswered:` instead, which also counts the attempts that reached the service and failed - `http-401` after a revoked key, a model mismatch, a malformed response - so the two kinds are named rather than only counted.
