...
State that no group owns and that `ledger.context` does not reach is copied verbatim into every part, so the split is taken only when the group-owned material is what makes the envelope oversized and the duplication stays inside one budget: the copied bytes must not exceed the group-owned bytes, and the copied bytes multiplied by the part count must not exceed the per-call budget.
