...
That header includes the optional `landing_branch=` field, which `bin/fm-landing-branch.sh` records or corrects on an existing task and `bin/fm-teardown.sh`'s landed-work test consumes.
