...
The three no-regression guards - the unresolvable landing-branch refusal, the caller-supplied workspace, and the already-registered happy path - pass both on base and with the fix.
...
ok - fm-playbot-lanes: a created workspace whose landing branch is unresolvable refuses immediately with no retry
...
On 2026-09-24, the running Playbot 0.117.0 on Linux returned a stream envelope from `window.electronAPI.invoke('threads:getSnapshot', {threadId})` in a read-only CDP `Runtime.evaluate` probe of a fresh throwaway chat, archived after the probe.
The same probe's `app:metadata` result was `{name: "Playbot", version: "0.117.0"}`; the snapshot's top-level keys were `streamId`, `revision`, `state`, `turns`, and `subAgents`, with `agentStatus: "idle"`, `phase: {kind: "idle"}`, and all six required card and queue lists under `state`.
The sanitized captured envelope is `tests/fixtures/playbot-0.117.0-thread-snapshot.json`, exercised through `bash tests/fm-playbot-lanes.test.sh` for live-card reads, queue reads, post-action results, send and dispatch verdicts, and missing-field refusal.
The probe establishes the 0.117.0 `threads:getSnapshot` shape; write-channel return envelopes in the fixture are regression inputs, not a claim of separate live write-channel verification.
...
The exact focused regression command `bin/fm-test-run.sh tests/fm-playbot-lanes.test.sh` passed with Node v26.7.0 and ended `FM_TEST_SUMMARY total=1 failed=0 skipped_gate=0` after 261,994 ms.
...
ok - fm-playbot-lanes: dispatch refuses to send when Playbot persisted a different effort than requested
ok - fm-playbot-lanes: create_chat refuses when Playbot persisted a different effort than requested
ok - fm-playbot-lanes: null model and reasoningEffort count as omitted while a null model beside an effort is refused
ok - fm-playbot-lanes: dispatch refuses a worker profile on an existing chat before any IPC
