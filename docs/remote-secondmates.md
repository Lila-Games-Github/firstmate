...
SSH exit 255 and any inconclusive reply or recovery cleanup preserve the local route, task metadata, and watcher artifacts so the local retirement can be reconciled or retried.
After every refusal gate passes, successful retirement removes the task check, trust binding, PR-poll records, Playbot lane sidecar, and quarantine entries before retiring the local route and metadata.
