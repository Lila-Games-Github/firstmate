...
`bin/fm-jev.sh` enforces the per-call budget for every adapter, so no adapter can make an oversized request disappear. An over-budget envelope is split into one request per question, each carrying only the shared context its own question names. Splitting is taken only when it buys something: state that no single question owns is copied verbatim into every part, so a split is refused unless that copy is smaller than the per-question material it accompanies and unless every copy together still fits one per-call budget - an oversized worker report shared by five acceptance criteria would otherwise cost five near-identical requests, when one request shortened by a few kilobytes carries every criterion and essentially the same report - and it is refused again when the whole split would not fit the day's remaining calls and spend for that use. Every refusal sends one request instead. A request that still does not fit has its longest state string shortened and records `truncated=true`, and a question that cannot fit even alone is refused with `per-call-token-cap` written to the ledger. Every budget refusal is recorded the same way, so `bin/fm-jev-report.sh` names under `refusals:` why a use did nothing today.
