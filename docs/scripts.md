...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), Playbot-lane (`--lane`), scout, secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
...
| `fm-learning-candidate.sh` | Capture durable development incidents and curate, route, deduplicate, dispose, or summarize them without changing a destination |
...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk and holder liveness) for fm-lock.sh, the Claude Stop auto-arm, and the session-start nudge |
| `fm-playbot-session-lock.mjs` | Validate a Playbot Codex thread against its persisted project root for session-lock ownership |
| `fm-windows-harness-process.ps1` | Resolve and validate Windows host harness processes when MSYS cannot see their ancestry |
...
| `fm-playbot-lanes.mjs`   | Ensure, verify, serve, and invoke Playbot chat lanes with optional catalog-validated worker model profiles and guarded workspace retirement from a terminal or a Playbot controller chat, with routed Stop-hook wakes for chat callers |
...
| `fm-landing-branch.sh`   | Show or record an existing ship task's `landing_branch=` meta field under the task meta lock |
| `fm-lane-base-check.sh`  | Report whether a Playbot lane workspace's landing branch is safe to start from, by exit code, writing nothing |
