...
Both allocation points are gated, not just the crewmate one: the durable `treehouse get --lease` that [`bin/fm-home-seed.sh`](../bin/fm-home-seed.sh) takes for a secondmate home draws from the same pool whenever the project is the firstmate repo itself, so it refuses a leased home a live record still names before it seeds over it, and holds the lease rather than returning it - a return cleans and resets that copy, while a held lease is never handed out again.
