...
A remote secondmate's checks-green event has no attributable CI verdict on this host, so it stays awaiting verification rather than reporting done, and an absent CI verdict is never promoted to green.
...
That base is the ship task's recorded landing branch when `--landing-branch` was given, so a lane targeting a long-lived development branch starts from that branch's live tip, and origin's resolved default branch otherwise; when origin has no such branch the base is the project clone's local branch of that name, and a landing branch that resolves nowhere or cannot be verified refuses the spawn rather than falling back to the default branch.
...
The same record scan runs at ALLOCATION as well as return, because a crewmate slot's hold is a live process and a host restart frees every slot while the records naming them survive: [`bin/fm-spawn.sh`](../bin/fm-spawn.sh) refuses a freshly handed-out slot a live record still names, before it is claimed or re-prepared, and [`bin/fm-slot-record-lib.sh`](../bin/fm-slot-record-lib.sh) owns both that scan and the read of what Treehouse itself reports for a slot.
The same scan also gates a RELAUNCH, which allocates nothing but puts a fresh agent into the copy a record already names: [`bin/fm-control.sh`](../bin/fm-control.sh) asks it as the relaunch transaction's first step, so a refusal leaves the running agent it protects untouched, and [`bin/fm-spawn.sh`](../bin/fm-spawn.sh) asks it again for a direct caller.
Both allocation points are gated, not just the crewmate one: the durable `treehouse get --lease` that [`bin/fm-home-seed.sh`](../bin/fm-home-seed.sh) takes for a secondmate home draws from the same pool whenever the project is the firstmate repo itself, so it refuses a leased home any task record still names - including a record that already holds the seed's own id, which is refused up front because one id cannot be both a task and a secondmate route.
A refused seed never returns the lease it took: `treehouse return` cleans, resets and terminates the processes under the worktree, which is the work the refusal protects, and treehouse offers no release that does less, so the lease is held - which also keeps a third allocation off that copy - and released by hand once the records are reconciled.
That held lease is not left silent: session start reports a slot whose record and pool have drifted in either direction, a crewmate copy a record names that the pool reports free, and a copy any record names - crewmate worktree or secondmate home - that carries a durable lease under a label that is not that record's own, with the release command for the second.
Both verdicts are derived from one pool read per record, so they can never be taken from two different snapshots.
A home whose records and pool have already drifted apart is reported at session start rather than left to the next dispatch, and a slot that two records are already stuck on is reconciled through `bin/fm-teardown.sh --reconcile-slot`, whose header owns the proofs it requires and which discards nothing and never touches the slot.
...
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, slot-ownership proof, endpoint-close refusal, PR-discovery fallback, pre-teardown run conclusion, stale-lock recovery procedure, and path-alias tolerant treehouse return; [runtime backend verification](verification/runtime-backends.md#treehouse-return-path-matching) owns the active evidence for that return matching; [`tests/fm-teardown-endpoint-safety.test.sh`](../tests/fm-teardown-endpoint-safety.test.sh) and [`tests/fm-secondmate-safety.test.sh`](../tests/fm-secondmate-safety.test.sh) pin the slot-collision boundary and the reconciliation path, [`tests/fm-spawn-worktree-settle.test.sh`](../tests/fm-spawn-worktree-settle.test.sh) pins the allocation-side refusal, and [runtime backend verification](verification/runtime-backends.md#treehouse-slot-status-is-a-live-process-reading) owns the evidence that a slot's reported status is a live-process reading.
...
The session-start fleet digest consumes the command's bounded summary; the command trusts record content over its derived lifecycle suffix and retains only the hard-bounded unresolved sample it displays, with no durable summary index or separate repair pass.
