...
That held lease is not left silent: session start reports a slot whose record and pool have drifted in either direction, a slot a record names that the pool reports free, and a slot a record names that carries a durable lease for someone else, with the release command for the second.
