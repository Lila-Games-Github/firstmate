...
Before printing, the wrapper uses `bin/fm-session-lock-lib.sh`, the same session-lock identity owner as `bin/fm-lock.sh`.
That shared check covers the ordinary harness ancestry walk and the Windows Playbot thread sidecar, so a shared Codex app-server pid never makes two Playbot chats look like one lock owner.
If the lock belongs to the exact current harness session, session start already ran and the wrapper stays silent.
