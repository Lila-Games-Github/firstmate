...
A queued or sending verdict advances only when Playbot's persisted ledger binds the exact message to a turn on that worker session, or when same-worker `drop_queued_message` receives `not-recallable` for that exact message, while bare queue disappearance remains armed because Playbot's UI may have recalled the message.
