...
An optional `registryProject` names the project in the controller home's firstmate registry, resolved through `bin/fm-project-mode.sh`; only a registered `local-only` posture whose main clone is a root of the selected Playbot project makes that clone's local `refs/heads/<landingBranch>` the landing evidence instead, because such a project lands without pushing and its remote branch lags.
...
Local workspaces, a workspace with no persisted roots, unreadable Git state, an unresolvable landing branch, a `working` or `pending_input` chat, any missing or unrecognized unarchived chat state, a live firstmate task record naming the workspace, an ahead commit, a tracked modification outside Playbot's exact churn allowlist, a POSIX executable-mode change hidden by `core.fileMode=false`, assume-unchanged or skip-worktree index flags, replacement refs, and an in-progress merge, rebase, cherry-pick, revert, or sequencer are blocking evidence rather than a bare false verdict.
...
Because Playbot prunes every stale registration when it cannot find an orphan through Git, any orphan also blocks as `prune-would-drop-unlanded-head` while another missing detached worktree's recorded HEAD is not proven landed.
...
An optional `discardLocalChanges` object records a captain's explicit authorization for that one workspace: `authorization` holds the captain's words, `allow` names the discardable blocker codes `tracked-modifications`, `untracked-files`, `ignored-files`, `orphaned-files`, or `unlanded-commits`, and `commits` must list every exact unlanded commit id exactly when `unlanded-commits` is allowed.
...
It repeats the complete inspection immediately before action and calls Playbot's own `workspace:delete` IPC with `preserveWorktrees: false`; Playbot removes registered worktrees, while the tool never changes Playbot's database itself.
...
After Playbot reports success and its workspace rows are gone, the tool itself removes each present orphan directory, only while it is still an orphan inside Playbot's storage, not a symlink or registered worktree, and matches the inspected inventory fingerprint, because Playbot finds worktrees through Git and cannot remove it.
It then verifies that the `workspaces` row, every `workspace_roots` row, worktree directory, and Git worktree registration are gone, then deactivates every durable lane route naming the workspace and serializes a complete mode-0600 private audit record under the lane state directory.
...
That record names the time, project, workspace, workspace paths, exact root heads, explicit landing branch and verified landing commits with their remote or local evidence, the discard authorization verbatim with every discarded tracked, untracked, and ignored path and unlanded commit, every removed orphan file, affected lane routes, IPC outcome, and removal verification.
...
A lane's instructions are generated by `bin/fm-brief.sh --lane`, whose `--help` owns that scaffold's syntax and what lane mode changes; a lane brief is written to need no dispatch-time override, so the dispatched task should point at the brief and describe the work rather than restate its branch, base, or delivery rules.
