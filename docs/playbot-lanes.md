...
That record holds the observed status and `updated_at`, Playbot's task-specific delivery verdict, worker and message identities, and a task acceptance boundary initialized from `last_user_activity_at` and advanced by exact-message acceptance evidence.
...
A queued or sending verdict advances only when Playbot's persisted ledger binds the exact message to a turn on that worker session, that session's persisted rollout records the exact client message id after the task acceptance boundary, or same-worker `drop_queued_message` receives `not-recallable` for that exact message, while bare queue disappearance remains armed because Playbot's UI may have recalled the message.
