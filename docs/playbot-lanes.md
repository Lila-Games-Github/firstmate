...
A queued or sending verdict advances only when that worker session's persisted rollout records the exact client message id after the task acceptance boundary.
A ledger turn binding or same-worker `not-recallable` outcome without that exact timestamp leaves the prior verdict armed and promotable by a later poll, while bare queue disappearance remains armed because Playbot's UI may have recalled the message.
