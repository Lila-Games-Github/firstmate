...
A populated submodule that cannot be inspected blocks retirement, as does a persisted submodule stash, staged index change, index flag, in-progress Git operation, or ref-, reflog-, or commit-pseudoref-reachable commit, including every structured `FETCH_HEAD` row, not proven reachable from a stable fresh snapshot of the configured remotes.
...
Durable route mutations share one cross-process lock whose complete owner record is atomically published with acquisition and includes a timezone- and locale-independent process start identity, so dead owners and reused PIDs recover without letting one generation reclaim or release another's lock.
