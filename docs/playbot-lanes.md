...
That record holds the pre-send status and the row's `updated_at` together with Playbot's task-specific delivery verdict and message identity.
...
Only Playbot's explicit `delivered` verdict permits terminal retirement, because an empty aggregate queue and generic worker activity cannot distinguish delivery from recall of the exact queued or sending message.
